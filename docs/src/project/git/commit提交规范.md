...
就像我上面说的一样，提交的代码多了，提交信息就会非常混乱，以至于要去查看分支才知道自己写了什么。如果是一个团队，那么提交信息就会更加混乱 😵‍💫，所以我们需要一个规范来规范提交信息。同时，规范的提交信息可以提供更多历史信息，方便快速浏览和过滤。

### Commit Message 的"骨骼结构"
...
## 来，给你的 Git 提交装上"门卫"：Husky

光有规范还不行，有时我们会忘记遵守。这时候 `husky` 就登场了！ 它就像一个 Git Hook 的管理器，能在 Git 生命周期的特定时机（比如提交前 `pre-commit`，提交信息前 `commit-msg`）执行一些脚本。
...
然后你就会看到一个友好的交互式界面，一步步引导你填写规范的提交信息，最后会自动生成漂亮的提交信息。例如：
